...
class DeviceInfoService {
  ...
  Future<void> init() async {
    try {
      ...
      _versionWithBuildNumber = Platform.isWindows
          ? _version
  ...
  Future<void> _initForAndroid() async {
    ...
    final model =
        'Model: ${info.model} --- Device: ${info.device} --- Manufacturer: ${info.manufacturer}';
    _setDefaultDeviceInfoProps(model, '${info.version.sdkInt}');
    _deviceInfo.putIfAbsent(
      'IsPhysicalDevice',
  ...
  Future<void> _initForIOS() async {
    ...
    final model =
        'Model: ${info.model} --- Device: ${info.utsname.machine} --- Manufacturer: Apple Inc';
    _setDefaultDeviceInfoProps(model, info.systemVersion);
    _deviceInfo.putIfAbsent(
      'IsPhysicalDevice',
      () => '${info.isPhysicalDevice}',
    );
  ...
  Future<void> _initVersionTracker() async {
    ...
    await vt.track();
    _versionChanged = vt.isFirstLaunchForCurrentBuild ??
        vt.isFirstLaunchForCurrentVersion ??
        vt.isFirstLaunchEver ??
        false;
  }

  void _setDefaultDeviceInfoProps(String model, String osVersion) {
    _deviceInfo
      ..putIfAbsent('Model', () => model)
